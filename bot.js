...
// ════════════════════PLUGGINS SUCCESS◽◽◽◽
        console.log(
            chalk.green.bold('🤖Public Bot working!')
       );
        
         if (config.LANG == 'EN') {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/Amazone.png"), MessageType.image, { caption: `🛸🚀Hey..!!  ${conn.user.name}! \n*🤖Welcome To Public Bot :|🌼*\n Your Bot Working  As ${config.WORKTYPE} 🤖.\n\n*🛸Public Bot WORKING Your Account*\n*🤖Use the 🚀.basichelp command to get a full understanding of the Public Bot testimonial...*\n*🤖Public Bot is a powerfull WhatsApp robot developed by insaaf.*\n*🚀 This is your LOG number. Avoid using the command here.*\n\n`});
             
         } else if (config.LANG == 'SI') {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/Amazone.png"), MessageType.image, { caption: `🛸🚀Hey..!!  ${conn.user.name}! \n*🤖සාදරයෙන් Public Bot වෙත පිලිගන්නවා :|🌼*\n\n ඔබේ Bot ${config.WORKTYPE} ලෙස ක්‍රියාකරයි.\n*🛸Public Bot ඔබගේ ගිණුමේ දැන් සක්‍රියයි*\n*💠Public Bot පිළිබද සම්පූර්ණ අවබෝධයක් ලබා ගැනීමට 🔶.basichelp විධානය භාවිතා කරන්න...*\n*🌟Public Bot යනූ සීඝ්‍රයෙන් වර්ධනය වන Whatsapp රොබෝවෙකි..Alexa වෙත ලැබෙන නව අංග හා යතාවත්කාලින කිරිම් ලබා ගැනීමට 🔶 .newslist විධානය භාවිතා කරන්න..*\n*🚀මෙය ඔබගේ LOG අංකයයි.මෙහි විධාන භාවිතයෙන් වළකින්න.*\n\n`});
    ...
    setInterval(async () => { 
        if (config.AUTOBIO == 'true') {
            if (conn.user.jid.startsWith('90')) { 
                ...
                const biography = '📅 ' + utch + '\n⌚ ' + ov_time + '\n\n⏱ Auto Bio By... 🚀powered By Public Bot'
                ...
                const biography = '📅 ' + utch + '\n⌚ ' + ov_time +'\n\n⏱ Auto Bio B... 🚀powered By Public Bot'
                await conn.setStatus(biography)
            }
...
// ════════════════════AUTO BIO◽◽◽◽◽    
    setInterval(async () => { 
        ...
        while (getGMTh == 19 && getGMTm == 1) {
            var announce = ''
            if (config.LANG == 'EN') announce = '📢◉◉🤖Public Bot◉◉ \n👾Announcement SYSTEM 🔘'
            if (config.LANG == 'SI') announce = '📢◉◉🤖Public Bot◉◉ \n👾නිවේදන පද්ධතිය 🔘'
            if (config.LANG == 'ID') announce = '📢◉◉🤖Public Bot◉◉ \n👾Announcement System 🔘'
            
            let video = 'https://c.tenor.com/hE0T8D0GpXsAAAAC/joinblink-blink.gif'
            let image = 'https://d2vrvpw63099lz.cloudfront.net/whatsapp-bots/whatsapp-bots.png'
...
*/
                        try {
                            ...
                            if (config.LANG == 'EN') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/Amazone.png"), MessageType.image, { caption: '*🚀Public Bot*  WORKING PERFECTLY !!\n\n▷ _This is your LOG number Dont Try Command here_\n▷Also You Can join Our Support group More Help.\n_👾Support 01▷https://chat.whatsapp.com/DSX2aegJpVRG3cWIUlBa48\n\n*Error:* ```' + error + '```\n\n' });
                                
                            } else if (config.LANG == 'SI') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/Amazone.png"), MessageType.image, { caption: '*🚀Public Bot*  නිසි ලෙස ක්‍රියා කරයි!!\n\n▷ _මෙය ඔබගේ LOG අංකයයි මෙහි විධන භාවිතයෙන් වළකින්න_\n▷ඔබට යම් ගැටලුවක් ඇත්නම් අපගේ සහය සමූහට ලිවිය හැක.\n_👾Support 01▷https://chat.whatsapp.com/DSX2aegJpVRG3cWIUlBa48\n\n*දෝෂය:* ```' + error + '```\n\n' });
